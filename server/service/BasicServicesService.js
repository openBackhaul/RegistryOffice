...
/**
 * @description Initiates process of embedding a new release
 *
 * @param {String} body V1_bequeathyourdataanddie_body 
...
exports.embedYourself = function (body, user, originator, xCorrelator, traceIndicator, customerJourney, originalUrl) {
  return new Promise(async function (resolve, reject) {
    try {
       ...
       * Prepare attributes and configure logical-termination-point
       ****************************************************************************************/
      let operationList = [replayServerReplacementOperation, deregisterOperation];
 ...
 * @param {String} traceIndicator String Sequence of request numbers along the flow
 * @param {String} customerJourney String Holds information supporting customer’s journey to which the execution applies
 * @returns returns inline_response_200_3
 **/
...
exports.informAboutApplicationInGenericRepresentation = function (user, originator, xCorrelator, traceIndicator, customerJourney) {
  return new Promise(async function (resolve, reject) {
    ...
    try {
      ...
      let requestForReleaseHistory = baseUrl + await operationServerInterface.getOperationName("ro-0-0-1-op-s-2004");
 ...
 * @param {String} traceIndicator String Sequence of request numbers along the flow
 * @param {String} customerJourney String Holds information supporting customer’s journey to which the execution applies
 * @returns {List} information about the application will be returned
 **/
 ...
 * @param {String} traceIndicator String Sequence of request numbers along the flow
 * @param {String} customerJourney String Holds information supporting customer’s journey to which the execution applies
 * @returns returns inline_response_200_7
 **/
...
/**
 * @description Initiates announcing of a new release of the RegistryOffice to the currently active RegistryOffice
 * Shall also automatically execute without receiving any request every time the application starts<br>
 *
  ...
  return new Promise(async function (resolve, reject) {
    try {
      ...
      if (createdOperationInstanceInformationList.length > 0) {
        ...
        let embedYourselfOperation = await operationServerInterface.getOperationName("ro-0-0-1-op-s-0001");
        let updateClientOperation = await operationServerInterface.getOperationName("ro-0-0-1-op-s-0007");
        let attributeList = [{
...
exports.startApplicationInGenericRepresentation = function (user, originator, xCorrelator, traceIndicator, customerJourney) {
  return new Promise(async function (resolve, reject) {
    ...
    try {
      ...
      let requestForListRegisteredApplication = baseUrl + await operationServerInterface.getOperationName("ro-0-0-1-op-s-3004");
      let requestForInformAboutApplication = baseUrl + await operationServerInterface.getOperationName("ro-0-0-1-op-s-2002");
      let consequentActionForListRegisteredApplication = new consequentAction(LabelForListRegisteredApplication, requestForListRegisteredApplication);
    ...
    try {
      ...
      if (layerProtocolName == layerProtocol.layerProtocolNameEnum.OPERATION_SERVER) {
        if (oldOperationKey == await operationServerInterface.getOperationKey(operationUuid)) {
          await operationServerInterface.setOperationKey(operationUuid, newOperationKey);
        }
      } else {
        if (oldOperationKey == await operationClientInterface.getOperationKey(operationUuid)) {
          await operationClientInterface.setOperationKey(operationUuid, newOperationKey);
        }
      }
    } catch (error) {
      console.log(error);
